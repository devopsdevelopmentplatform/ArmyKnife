...
function use() {
        ...
        if [ "${WITH_DOCKER}" == "true" ]; then
                ...
                echo "# Enabling docker support. Be sure you install a docker CLI binary (apt-get install -y docker.io)."
        ...
        if [ -n "$container_id" ]; then
                ...
                docker run --network=bridge "${DOCKER_ARGS[@]}" ${GEODESIC_DOCKER_EXTRA_ARGS} ${DOCKER_IMAGE} -l $*
        fi
}
