...
if [ ! -d "/home/developer/docker/buildx/cache" ]; then
  mkdir -p /home/developer/docker/buildx/cache
fi
...
#echo $CI_DOCKER_REGISTRY_PASSWORD | docker login -u $CI_DOCKER_REGISTRY_USER --password-stdin $REGISTRY_DOCKER
...
if [ "$REGISTRY_DOCKER" = "index.docker.io/v1/" ]; then
    echo "Building for Docker Registry"
    REGISTRY=$REGISTRY_DOCKER docker buildx bake --progress plain --push -f bake.hcl default
fi
